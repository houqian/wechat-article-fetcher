...
//dictht哈希表的结构如下：dictEntry **table; //哈希结点数组
long size; //哈希表大小
long used; //哈希结点数量
long sizemask; //哈希表大小掩码，用于计算索引值

//dictEntry哈希结点的结构如下：void *key; //键，8字节
union v; //值，8字节
...
//zskiplistNode跳跃表结点的结构如下：*backward: 后退指针
score: 分值
obj: 成员对象
zskiplistLevel: 层数组

//zskiplistLevel的结构如下：*forward: 前进指针
span: 跨度
```

### (2)跳跃表的图示

![图片](assets/21ea43ca6caa.png)

![图片](assets/8f4e1b630d8f.webp)
...
//压缩列表结点的结构如下：previous_entry_length: 前一个节点长度，实现表尾到表头的遍历，占1或5个字节
