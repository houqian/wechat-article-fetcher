...
![图片](assets/867b02209a9b.png)

epoll版的多路复用：

![图片](assets/4181187e70a6.png)
...
ASK错误指的是：节点的槽位正在迁移，但却收到了一个key请求，此时该节点没能在自己的数据库里找到该key。于是该节点就会看一下ClusterState.migration_slots_to，看看key所属的槽位是否发生迁移。如果是，则响应客户端ASK错误并引导客户端到正在导入槽位的节点去查找key。
