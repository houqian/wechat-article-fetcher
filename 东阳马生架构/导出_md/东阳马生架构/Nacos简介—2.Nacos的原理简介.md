...
新加入Nacos集群的节点会轮询Nacos集群的所有节点，然后发送请求出去拉取各节点的数据，所以Nacos集群的每个节点都会有所有已注册服务实例的数据。
...
然后，Nacos集群节点收到写请求后：就会根据需要注册的服务实例的IP:端口 \+ 路由算法，计算出所属的集群节点。

接着，把服务实例注册请求转发到负责该服务实例数据的集群节点中。负责该服务实例数据的节点会解析请求，缓存服务实例数据到内存中。

![图片](assets/c8aa34f1ca53.png)
