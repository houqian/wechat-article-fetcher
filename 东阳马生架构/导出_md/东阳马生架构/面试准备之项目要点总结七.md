...
- 1.基于Canal和RocketMQ的增量同步
- 2.增量同步任务的背景介绍
- 3.增量同步任务查询与线程池提交
- 4.RocketMQ里的binlog消息的消费逻辑分析
- 5.新增binlog的数据同步逻辑分析
- 6.binlog基于内存队列的异步转发逻辑
- 7.基于CAS加锁的读写队列互换机制
- 8.binlog基于内存的merge合并逻辑
- 9.对merge数据从目标库里分批查询
- 10.对merge数据基于目标库数据做过滤
- 11.将过滤后的merge数据写入目标库
- 12.offset提交线程的启动和逻辑分析
- 13.增量同步过程中binlog写入失败的恢复
- 14.增量同步过程中的各种失败场景的恢复机制
- 15.定时移除已提交的增量同步消息
- 16.增量与全量并行运行的场景分析
- 17.增量与全量并发同步一批数据的冲突
- 18.同步完成后的数据校验逻辑分析
        ...
        if (JSONUtil.isJson(binlogStr)) {
            ...
            if (null != dataArray) {
                ...
                if (null != dataArrayIterator) {
                    ...
                    } elseif (DATATYPE_MAP.equals(dataType)) {
...
public class CanalPullRunner implements Runnable {
    ...
    private void process NewMsg(MessageExt messageExt, String msg){
        ...
        consumeRecordMapper.insert(consumeRecord);
        LocalQueue.getInstance().submit(binlogData, consumeRecord);
        ...
...
public class MergeBinlogWrite{
    ...
    private Map<String, Map<String, Object>> batchQuery(OperateType operateType, String domain) {
        ...
        } elseif (operateType == OperateType.ALL) {
            //全量数据进行分组(只是模型转换的概念)
    ...
    public void filterBinlogAging(OperateType operateType, String domain) {
        ...
        for (Map.Entry<String, BinLog> entry : binlogDataMap.entrySet()) {
            ...
            if (!CollectionUtils.isEmpty(respMap) && respMap.containsKey(entry.getKey())) {
                ...
                } elseif (MigrateCheckUtil.comparison(binLogMap, targetMap, tableName)) {
...
public class RemoveBinlogConsumeTask {
    ...
    public void removeBinlogConsumeRecordTask() {
        //每次删除超过当前时间5分钟的历史数据
        Date updateTime = DateUtils.addMinute(newDate(), -5);
