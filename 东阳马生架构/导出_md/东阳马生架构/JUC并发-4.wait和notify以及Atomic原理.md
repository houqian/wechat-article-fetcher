...
在内存的文件目录树中创建一个目录节点的代码如下所示。由于内存里的文件目录树是会被多线程并发写的资源，所以创建目录的代码块必须要用synchronized保护起来。
...
要保证多个变量的原子性，可以使用JDK提供的AtomicReference来封装自定义对象。将多个变量放在一个对象里，通过对象的引用来实现多个变量的原子性。
...
由于服务注册表的每一条数据并不是都会变化的，每隔30秒可能只有少数几个服务实例的数据出现变化，所以不需要每隔30秒就全量拉取服务注册表的所有数据。
