...
首先通过Redis的hexists命令判断在key为锁名的Hash值里，field为UUID + 线程ID的映射是否已经存在了。
...
异步执行lua脚本完成后，会传入布尔值来触发future.whenComplete()方法的回调。在回调中，会根据布尔值，来决定是否继续递归调用renewExpiration()方法。
...
所以，持有锁的线程每重入锁一次，就会对field映射的value值递增1。比如执行命令：hincrby key UUID:ThreadID 1。
...
于是就利用同步组件Semaphore进行阻塞等待一段ttl的时间。阻塞等待一段ttl时间后，会继续执行while循环里的逻辑，也就是再次尝试获取锁。以此循环往复，直到获得锁。
...
当然，前提是创建锁的时候没有设置锁的存活时间leaseTime。如果指定了leaseTime，那么就不会存在10秒后检查锁的定时调度任务。所以如果获取锁的所在机器宕机，那么锁对应的key会在最多leaseTime后过期。
...
在while循环中不断地尝试获取锁。使用time表示还可以等待获取锁的时间。每次循环都对time：自减尝试获取锁的耗时 \+ 自减每次等待的耗时。如果在指定时间内没有成功加锁(即time小于0)，那么就退出循环，表示加锁失败。
