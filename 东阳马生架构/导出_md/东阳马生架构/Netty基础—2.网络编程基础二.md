...
接着Acceptor处理器通过accept()方法便能得到这个客户端对应的连接(SocketChannel)，然后再将这个连接传递给subReactor线程池进行处理。

subReactor线程池会分配一个subReactor线程给这个SocketChannel，并将SocketChannel关注的read事件及对应的read事件处理器注册到这个subReactor线程中。当然也会注册关注的write事件及write事件处理器到subReactor线程中以完成IO的写操作。
