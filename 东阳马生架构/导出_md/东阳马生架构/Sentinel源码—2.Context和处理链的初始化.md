...
已知资源可以是一个方法、一个接口甚至一段代码，那么资源就肯定要有资源名称，比如方法名、接口名、其他自定义名称，因此可设计如下Entry资源访问类：
...
因此一个资源起码有三个字段：名称(name)、资源类型(resourceType)以及请求类型(entryType)。将这三个字段包装成一个ResourceWrapper类，如下：
...
ClusterBuilderSlot负责实现集群限流功能，ClusterBuilderSlot会将请求的流量信息汇报到集群的统计节点，然后根据集群限流规则，来决定是否应该限制请求。
...
SystemSlot用于实现系统保护功能，它会提供基于系统负载、基于系统平均响应时间和基于系统入口QPS的保护策略。
...
每次访问资源都要创建一个Entry对象，资源操作要建立在一个Context环境下。一个请求对应一个Context，一个Context可以包含多个资源，也就是一个Context会包含一条完整请求链路中涉及的所有资源。每个资源都通过ProcessorSlot进行数据采集和规则验证，而采集完的数据则会交由Node去做聚合统计分析。
...
由于一个请求要与一个Context对象进行绑定，一个请求由一个线程处理。所以可以定义一个ThreadLocal变量，让一个线程与一个Context对象绑定，如下所示：
...
处理链(处理器插槽链条)ProcessorSlotChain其实就是一条责任链，由于责任链是单向链表结构，所以每个Slot必须要有一个next属性，用于指向下一个节点。
