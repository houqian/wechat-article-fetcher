...
所以前一个节点发生变化时，便会通知添加的Watcher监听。然后便会唤醒阻塞的线程，继续执行internalLockLoop()方法的while循环。而while循环又会继续获取排序的节点列表 \+ 判断当前线程是否已获取锁。
