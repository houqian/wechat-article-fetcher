...
答：一个内存分配器里维护着一个PoolArena数组，所有的内存分配都是在PoolArena上进行的。Netty会通过一个PoolThreadCache对象将线程和PoolArena进行一一绑定(利用ThreadLocal原理)。默认一个线程对应一个PoolArena，这样就能做到多线程内存分配相互不受影响。
...
Netty的ByteBuf是通过引用计数的方式管理的，如果一个ByteBuf没有被其他地方引用到，则需要回收其底层内存。
...
Unsafe和非Unsafe不需要我们操心，Netty会根据系统是否有unsafe对象自行选择。JDK里有个Unsafe对象，它可以直接拿到对象的内存地址，然后可以基于这个内存地址进行读写操作。Unsafe类型的ByteBuf可以拿到ByteBuf对象在JVM里的具体内存地址，然后直接通过JDK的Unsafe类进行读写。非Unsafe类型的ByteBuf则是不会依赖到JDK里的Unsafe对象。
