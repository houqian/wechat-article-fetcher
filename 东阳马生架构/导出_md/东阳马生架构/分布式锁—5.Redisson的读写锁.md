...
然后接着执行命令"del {myLock}:UUID1:ThreadID1:rwlock_timeout:1"，也就是删除用来记录当前客户端线程第1个重入锁过期时间的key。最后获取每个重入读锁和非重入读锁的剩余过期时间，并找出其中最大的。执行"pexpire myLock"重置读锁的过期时间，为最大的剩余过期时间。
