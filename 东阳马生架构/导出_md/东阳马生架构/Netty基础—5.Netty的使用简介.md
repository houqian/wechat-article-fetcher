...
option()方法可以给服务端Channel也就是NioServerSocketChannel设置TCP参数，比如b.option(ChannelOption.SO_BACKLOG, 1024)就是设置临时存放已完成三次握手的请求的队列的最大长度为1024。

#### 六.ServerBoostrap的childOption()方法

childOption()方法可以给每一个连接都设置一些TCP参数，其中ChannelOption.SO_KEEPALIVE表示的是是否开启TCP底层的心跳机制，而ChannelOption.TCP_NODELAY表示的是是否开启Nagle算法。如果要求实时性则可以关闭Nagle算法，如果要求减少发送次数和网络交互次数则可以开启Nagle算法。
...
第一次读取到了完整的D1包和D2包的部分内容，第二次读取到了D2包的剩余内容，于是出现了TCP拆包的情况。

情况四：服务端分两次读取到了两个数据包

第一次读取到了D1包的部分内容，第二次读取到了D1包的剩余内容和完整的D2包，于是出现了TCP拆包的情况。
...
服务端在收到数据包后会将其放到缓冲区中，如果数据包没有及时从缓存区中取走，那么下次获取数据包的时候就可能会出现一次取出多个数据包的情况，从而产生粘包问题。
...
MTU指的是一种通信协议的某一层上面所能通过的最大数据包大小。如果IP层有一个数据包要传，而且数据的长度比链路层的MTU大，那么IP层就会进行分片，把数据包分成若干片，让每一片都不超过MTU。注意IP分片可以发生在原始发送端主机上，也可以发生在中间路由器上。
...
它的工作原理就是依次遍历ByteBuf中的可读字节，然后判断是否有"\n"或者"\r\n"。如果有，那么就以此位置为结束位置，从可读索引到结束位置区间的字节就组成了一行。

它是以换行符为结束标志的解码器，支持配置单行的最大长度。如果连续读取到最大长度后仍然没有发现换行符，则抛出异常，并忽略之前读取到的异常码流。
...
![图片](assets/7b4c2075e114.png)

于是就可以根据长度域偏移量和长度域的长度来构造一个解码器，第一个参数是数据包的最大长度，第二个参数是长度域的偏移量，第三个参数是长度域的长度。
