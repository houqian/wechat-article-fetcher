...
执行命令"get semaphore"，获取到semaphore的当前值。如果semaphore的当前值不是false，且大于客户端线程申请获取锁的数量。那么就会执行命令"decrby semaphore 1"，将信号量允许获取锁的总数量递减1。
...
在while循环中，首先会执行命令"get myCountDownLatch"去获取countDown值。如果该值不大于0，那么就退出循环不阻塞线程。如果该值大于0，那么就说明还没有指定数量的线程去执行countDown操作，于是就会先阻塞线程，然后再等待唤醒来继续循环。
