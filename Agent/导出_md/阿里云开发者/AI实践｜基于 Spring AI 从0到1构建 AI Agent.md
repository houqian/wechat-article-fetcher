...
`SkillManager` 在启动时扫描 `classpath:skill/*.md`，解析元数据后由 `SkillTool` 将每个技能转换为 `ToolCallback` 注册到 Agent。LLM 在对话中根据 `description`自主判断是否需要调用某个技能。
