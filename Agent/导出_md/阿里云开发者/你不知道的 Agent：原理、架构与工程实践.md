...
![Agent Loop 控制流](assets/a1511e36ffa8.svg)
...
![Workflow 与 Agent 对比](assets/6c391a74ede8.svg)
...
![五种常见控制模式](assets/cdc201e47bc0.svg)
...
![Codex 可观测性栈](assets/b67419626fc7.svg)
...
![Harness 关键结论](assets/5eb2aba13d48.svg)
...
![上下文分层结构](assets/6b13efd9d919.svg)
...
![Skills 按需加载](assets/f94eddeccc1e.webp)
...
![ACI 工具设计对比：差工具设计会让 Agent 反复绕圈，好工具设计能让 Agent 更快选对并修正错误](assets/1c572212d5df.svg)
...
![四种记忆类型与存储位置：上下文窗口位于运行时 messages\[\]，Skills、JSONL 会话历史和 MEMORY.md 位于磁盘，生命周期和注入方式各不相同](assets/4a6f29076e52.svg)
...
![记忆整合与回退流程：消息流在 token 使用率超过阈值后触发整合，成功时摘要写入 MEMORY.md 并移动整合指针，失败时原始消息写入 archive/ 保留完整历史](assets/afa4ad162f25.svg)
...
![Initializer + Coding Agent 跨 session 协作流程：Initializer 只运行一次并生成 feature-list.json、init.sh、初始 commit 和 claude-progress.txt，后续 Coding Agent 在多个 session 中通过文件系统恢复状态、实现单个功能、测试、更新 passes 并提交代码](assets/058a2a3a3839.svg)
...
![多 Agent 协作协议](assets/b3161524f82f.svg)
...
![多 Agent 幻觉放大](assets/dce0793543be.svg)
...
![Single-turn vs Agent 评测对比：Single-turn 是 Prompt 进 LLM 出 Response 直接打分，Agent 则需要 Tools、Environment、Task 协同，Agent 多步调用工具并更新环境状态，最后验证环境实际结果而非只看输出文字](assets/a9093934fe06.webp)
...
![Agent 评测的组成部分：task、trial、grader、transcript、outcome、evaluation harness、agent harness 和 evaluation suite](assets/401cf5b36e52.webp)
...
![调查：团队实际使用的评测方式，Offline evaluation on test sets 54.5%，Online evaluation on production data 44.8%，Not evaluating yet 22.8%](assets/9db8a219c861.webp)| ![调查：常用评测指标，Internal human review/labelling 59.8%，LLM-as-judge 53.3%，Traditional ML/DS metrics 16.9%](assets/48e9f9b97fb6.webp)
---|---
...
![Success rate vs infra error rate：横轴是评测容器的资源余量从 1x 到 Uncapped，蓝色是模型得分，红色是基础设施错误率，资源越受限红色越高蓝色越低](assets/6365b40fd170.webp)
...
![两层可观测性](assets/e88894e1f964.svg)
...
![事件流可观测性](assets/4bca08ad0e26.svg)
...
![OpenClaw 整体架构](assets/f8561a6a31aa.svg)层| 实现| 主要职责| 关键设计决策
---|---|---|---
...
![系统提示分层叠加](assets/911d47becb94.svg)
